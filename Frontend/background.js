...
async function checkForPhishing(url, tabId, isReload = false) {
  try {
    ...
    const isTrusted = trustedDomains.some(trustedDomain => domain === trustedDomain || domain.endsWith("." + trustedDomain));
    if (isTrusted) {
      const result = {
        url,
        isPhishing: false,
...
cchrome.webNavigation.onCommitted.addListener((details) => {
  if (details.frameId === 0) {
    ...
    debouncedCheck(url, details.tabId, isReload);
  }
});
